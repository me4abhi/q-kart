...
const Checkout = () => {
  ...
  if (addresses.all.length === 0) {
    ...
    addressView = addresses.all.map((address) => {
      let addressListItemClassName;
      if (address._id === addresses.selected._id) {
        addressListItemClassName = "address-item selected";
      } else {
        addressListItemClassName = "address-item not-selected";
      }

      return (
        <Box key={address._id} className={addressListItemClassName}>
          <Typography
            onClick={(event) =>
              setAddresses({ ...addresses, selected: address })
            }
            my="1rem"
          >
            {address.address}
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Button onClick={() => performCheckout(token, items, addresses)} startIcon={<CreditCard />} variant="contained">
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
